...
describe 'program' do
  ...
  it 'arithmetic operator exec' do
    ...
    expect(prog.exec["eval_2"]["result"]).to eq 5
    ...
    expect(prog.exec["eval_1"]["result"]).to eq 3
    expect(prog.exec["eval_2"]["result"]).to eq 5
    expect(prog.exec["eval_3"]["result"]).to eq 3
    expect(prog.exec["eval_4"]["exec"]).to eq "((2+3)+3)"
    expect(prog.exec["eval_4"]["result"]).to eq 8

    prog = OoTalk::Program.new('[{"Program":{"Constant":{"Left": 15}}}]')
    expect(prog.exec["eval_0"]["result"]).to eq 15

    prog = OoTalk::Program.new('[{"Program":{"Substract":{"Left":{"Constant":{"Left": 5}}, "Right":{"Constant":{"Left":3}}}}}]')

    expect(prog.exec["eval_0"]["result"]).to eq 5
    expect(prog.exec["eval_1"]["result"]).to eq 3
    expect(prog.exec["eval_2"]["result"]).to eq 2

    prog = OoTalk::Program.new('[{"Program":{"Times":{"Left":{"Constant":{"Left": 2}}, "Right":{"Constant":{"Left":3}}}}}]')

    expect(prog.exec["eval_0"]["result"]).to eq 2
    expect(prog.exec["eval_1"]["result"]).to eq 3
    expect(prog.exec["eval_2"]["result"]).to eq 6
    ...
    expect(prog.exec["eval_2"]["result"]).to eq 8

    prog = OoTalk::Program.new('[{"Program":{"Logarithm":{"Left":{"Constant":{"Left": 2}}, "Right":{"Constant":{"Left":8}}}}}]')
    expect(prog.exec["eval_0"]["result"]).to eq 2
    expect(prog.exec["eval_1"]["result"]).to eq 8
    expect(prog.exec["eval_2"]["result"]).to eq 3

    prog = OoTalk::Program.new('[{"Program":{"Surplus":{"Left":{"Constant":{"Left": 10}}, "Right":{"Constant":{"Left":3}}}}}]')
    expect(prog.exec["eval_0"]["result"]).to eq 10
    expect(prog.exec["eval_1"]["result"]).to eq 3
    expect(prog.exec["eval_2"]["result"]).to eq 1
  end
  ...
  it 'boolean operator exec' do
    ...
    expect(prog.exec["eval_0"]["result"]).to eq 2
    expect(prog.exec["eval_1"]["result"]).to eq 3
    expect(prog.exec["eval_2"]["result"]).to eq true
  ...
  it 'instruction exec' do
    ...
    expect(prog.exec["eval_2"]["result"]).to eq 2
    expect(prog.exec["eval_5"]["result"]).to eq 5
  end
